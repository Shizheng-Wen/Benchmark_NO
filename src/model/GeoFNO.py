...
################################################################
# GeoFNO (SpectralConv2d and IPHI same as Elasticity)
################################################################
...
class Geo_FNO(nn.Module):
    ...
    configs = {
        ...
        's1':                   60,                     # Number of x-points on latent space GeoFNO grid
        's2':                   60,                     # Number of y-points on latent space GeoFNO grid
    ...
    def __init__(self, configs):
        ...
        self.conv3 = SpectralConv2d(self.width, self.width, self.modes1, self.modes2, self.s1, self.s2)
        self.w1 = nn.Conv2d(self.width, self.width, 1)
        self.w2 = nn.Conv2d(self.width, self.width, 1)
        self.b0 = nn.Conv2d(2, self.width, 1)
        self.b1 = nn.Conv2d(2, self.width, 1)
        self.b2 = nn.Conv2d(2, self.width, 1)
        self.b3 = nn.Conv1d(2, self.width, 1)
    ...
    def forward(self, x):
        ...
        u = self.conv3(uc, x_out=x_out, iphi=self.model_iphi, code=code)
        u3 = self.b3(x_out.permute(0, 2, 1))
        u = u + u3
